...
struct Lucy {
	...
	void show_response(int model, const char* filename){
	    ...
	    for(int i = 0; i < network[model].classes; i++)
	    	printf("Predicao %d: %f, ", i, predicao[x][i]);
	    printf("Resultado: %s\n", network[model].labels[resultado]);
	    for(int i = 0; i < network[model].classes; i++)
	    	printf("%s -> %.1f%%, ", network[model].labels[i], (predicao[x][i] * 100.0));
